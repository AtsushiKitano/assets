...
resource "google_folder" "gland_child_folder" {
  for_each = length(var.child_folders) > 0 ? { for v in var.gland_child_folders : "${v.parent_folder_name}/${v.name}" => v } : {}
...
resource "google_folder" "gland_glandson_folder" {
  for_each = length(var.gland_child_folders) > 0 && length(var.child_folders) > 0 ? { for v in var.gland_glandson_folders : "${v.parent_folder_name}/${v.name}" => v } : {}
